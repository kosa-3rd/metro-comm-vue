...
<template>
  <div id="buttonArea">
    <transition-group name="zoom" tag="div" class="grid grid-cols-3 gap-2 p-2">
      <template v-if="menus && menus.length > 0">
        <button
          ...
          :class="[
            'station-btn',
            'clickable',
            { active: activeStationId === menu.id },
          ]"
      ...
      <div v-else class="col-span-3 flex justify-center items-center">
        ...
        <img
          src="../../assets/NO_Route.png"
          alt="No Route"
          class="ml-2 w-20 h-20"
        />
      </div>
    </transition-group>
  </div>
  <div class="flex justify-end pr-2">
    <button
      @click="toggleMenu"
      class="toggle-btn text-gray-500 font-thin text-xs"
    >
      {{ isExpanded ? "접기 " : "펼치기" }}
    </button>
  </div>
...
export default {
  ...
  watch: {
    ...
    stationId: {
      ...
      handler(newVal) {
        console.log(
          "New stationId received in BoardTopStationBtnCmp.vue:",
          newVal
        ); // stationId가 올바르게 전달되는지 확인
        if (newVal) {
  ...
  methods: {
    ...
    handleOutsideClick(event) {
      ...
      if (
        ...
        event.target.classList.contains("station-btn")
      ) {
        return;
      }
